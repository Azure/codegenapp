...
export async function CompleteCodeGenerationTask() {
    ...
    let codegens: CodeGeneration[] = await ListCodeGenerationsByStatus(CodegenDBCredentials.server,
    ...
    for (let codegen of codegens) {
        ...
        if (codepr !== undefined && codepr.length > 0) {
            const isMerged: boolean = await IsMergedPullRequest(PipelineCredential.token, codepr);
            if (!isMerged) continue;
        }
        
        const swaggerpr = codegen.swaggerPR;
        if (swaggerpr !== undefined && swaggerpr.length > 0) {
            const isMerged: boolean = await IsMergedPullRequest(PipelineCredential.token, swaggerpr);
            if (!isMerged) continue;
        }
        ...
        const err = CodeGenerateHandler.CompleteCodeGeneration(PipelineCredential.token,

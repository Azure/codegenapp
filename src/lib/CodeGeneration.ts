...
export async function ListCodeGenerations(server: string, database: string, user: string, password:string,type: string, filterCompleted: boolean = false):Promise<CodeGeneration[]> {
    let codegens: CodeGeneration[] = [];
    var sql = require("mssql");
    var config = {
        user: user,
    ...
    try {
        ...
        for (let record of result.recordset) {
            if (filterCompleted) {
                if (record["status"] === CodeGenerationStatus.CODE_GENERATION_STATUS_COMPLETED ||
                record["status"] === CodeGenerationStatus.CODE_GENERATION_STATUS_PIPELINE_COMPLETED) continue;
            }
            const codegen = new CodeGeneration(record["resourceProvider"], 
                                                        record["sdk"],
                                                        record["type"],

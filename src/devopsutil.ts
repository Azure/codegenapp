...
export async function createRepository(serverUrl:string, project: string, reponame:string): Promise<GitRepository> {
    const createOptions: GitRepositoryCreateOptions = <GitRepositoryCreateOptions >{name: reponame};
    const gitApiObject = await getGitApi(serverUrl);
    let newRepo: GitRepository = await gitApiObject.createRepository(createOptions, project);

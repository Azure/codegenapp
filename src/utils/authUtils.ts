...
export class AuthUtils {
    ...
    public async getWhitelistedCerts(authMetadataEndpoints: string[]): Promise<string[]> {
            ...
            authMetadataEndpoints.map(async (endpoint) => {
                try {
                    ...
                    if (response && response.status === 200) {
                        ...
                        return body.clientCertificates ? body.clientCertificates.map((cert: any) => cert.thumbprint) : [];
                    } else {
                        this.logger.error(`Failed to get metadata from: ${endpoint}, got: ${JSON.stringify(response)}`);
                        return [];
                    }
                } catch (err) {
                    this.logger.error(`Failed to get metadata from: ${endpoint}, got: ${JSON.stringify(err)}`);
                    return [];
                }
            }),
        );
    ...
    public ensureAuth(req: express.Request, res: express.Response, next: express.NextFunction, customerThumbprints: CustomersThumbprints) {
        if (!req.path.includes(config.healthProbeEndpoint)) {
            ...
            const normalizedFingerprint = cert.fingerprint.toUpperCase().replace(/:/g, '');

            let found = false;
            // eslint-disable-next-line guard-for-in
            for (const key in customerThumbprints) {
            ...
            if (!found) {
                this.logger.warn(`Invalid client certificate for client incoming request ${normalizedFingerprint}.`);
    ...
    public async getCustomersThumbprints(customers: Customer[]) {
        const customerThumbprintsPairs: ReadonlyArray<[string, string[]]> = await Promise.all(
            customers.map(async (c) => [c.id, [...c.thumbprints, ...(await this.getWhitelistedCerts(c.authMetadataEndpoints))]] as [string, string[]]),
        );
    ...
    public async fetchWithRetry(url: string, init?: RequestInit): Promise<Response> {
        const rawResponse = await retry(
            async (bail, retryNr) => {
                ...
                this.logger.info(`Executing api call for ${maskedUrl}, retry nr: ${retryNr}`);
                ...
                if (isLastRetry) {
                    this.logger.error(`Failed REST call for url: ${maskedUrl} after all retries.`);
                }
                if ((!response || response.status > 399) && !isLastRetry) {
                    const responseBody = await response.text();
                    this.logger.warn(`Failed REST call nr ${retryNr} for ${maskedUrl}, response status:${response.status},body:${responseBody}`);

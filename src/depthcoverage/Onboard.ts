import {
  ...
  DepthCoverageType,
  AutorestSDK,
  QueryCandidateResources,
  QueryDepthCoverageReport,
  ConvertOperationToDepthCoverageResourceAndOperation,
  ConvertResourceToDepthCoverageResourceAndOperation,
  CandidateResource,
} from "./QueryDepthCoverageReport";
import {
  uploadToRepo,
  createPullRequest,
  getBlobContent,
...
export async function RetriveResourceToGenerate(
  ...
  const opOrresources: any[] = await QueryDepthCoverageReport(
    server,
    db,
    user,
    pw,
  ...
  if (
    ...
    const res: ResourceAndOperation[] = await ConvertOperationToDepthCoverageResourceAndOperation(
      opOrresources,
      sdk,
      supportedResources
    );
    ...
    const res: ResourceAndOperation[] = await ConvertResourceToDepthCoverageResourceAndOperation(
      opOrresources,
      sdk,
      supportedResources
    );
...
export async function DeleteAllDepthBranchs(
  token: string,
  org: string,
  repo: string
) {
...
export async function DeletePipelineBranch(
  token: string,
  org: string,
  repo: string,
  branch: string
...
export async function TriggerOnboard(
  dbserver: string,
  db: string,
  dbuser: string,
  dbpw: string,
  ...
  const tfcandidates = await QueryCandidateResources(
    dbserver,
    db,
    dbuser,
    dbpw,
  ...
  const tfresources = await RetriveResourceToGenerate(
    dbserver,
    db,
    dbuser,
    dbpw,
  ...
  const clicandidates = await QueryCandidateResources(
    dbserver,
    db,
    dbuser,
    dbpw,
  ...
  const cliresources = await RetriveResourceToGenerate(
    dbserver,
    db,
    dbuser,
    dbpw,
  ...
  for (let rs of resources) {
    try {
      ...
      if (alreadyOnboard) {
        console.log(
          "Already triggerred to onboard " + rs.RPName + ". Ignore this one."
        );
        continue;
      }
...
export async function ReadPRFiles(
  token: string,
  org: string,
  repo: string,
  prNumber: number
  ...
  const prdata = await getPullRequest(octo, org, repo, prNumber);
  const headbranch = prdata.head.ref;
  await readCurrentCommitContent(octo, org, repo, headbranch, [
    "ToGenerate.json",
    "readme.az.md",
    "schema.json",
  ]);
...
export async function SubmitPullRequest(
  ...
  let err = undefined;
  try {
    prlink = await createPullRequest(
      octo,
      org,
      repo,
      basebranch,

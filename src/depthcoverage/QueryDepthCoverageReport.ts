...
export async function QueryCandidateResources(
  server: string,
  database: string,
  user: string,
  password: string,
...
export async function QueryDepthCoverageReport(
  server: string,
  database: string,
  user: string,
  password: string,
...
export async function ConvertOperationToDepthCoverageResourceAndOperation(
  ops: Operation[],
  sdk: string,
  ...
  for (let op of ops) {
    ...
    if (
      ...
      !IsCandidateResource(supportedResource, serviceName, op.fullResourceType)
    )
      continue;
    /* use api-version in candidate. */
    let candidate = GetCandidateResource(
      supportedResource,
      serviceName,
      op.fullResourceType
    );
...
export async function ConvertResourceToDepthCoverageResourceAndOperation(
  ...
  for (let crs of resourcelist) {
    ...
    if (
      supportedResource !== undefined &&
      !IsCandidateResource(supportedResource, serviceName, crs.fullResourceType)
    )
      continue;
    let candidate = GetCandidateResource(
      supportedResource,

...
import {
  RetriveResourceToGenerate,
  DeletePipelineBranch,
  ReadPR,
  ReadPRFiles,
} from "./Onboard";
import {
...
async function GetResources(
  ...
  if (sdk === undefined) {
    ...
    if (sdk === AutorestSDK.AUTOREST_SDK_TF) {
      resources = await RetriveResourceToGenerate(
        dbserver,
        db,
        dbuser,
        dbpw,
...
async function SubmitPR(
  ...
  let resources = await RetriveResourceToGenerate(
    dbserver,
    db,
    dbuser,
    dbpw,
  ...
  for (let rs of resources) {
    try {
      ...
      await createPullRequest(
        ...
        basebranch,
        rs.RPName,
        "pull request from branch " + rs.RPName
      );
